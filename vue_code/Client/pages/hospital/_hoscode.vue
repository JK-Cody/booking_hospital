<template>
    ...
    <div class="sub-item" v-for="(item,index) in departmentVoList" :key="item.id" :class="index == activeIndex ? 'selected' : ''" @click="selecting(index,item.depcode)"> {{ item.depname }}</div>
    </div>
    </div>
...
<script>
    ...
    init() {
        ...
        hospitalApi.getDepartmentByHosname(this.hoscode).then(response => {
        this.departmentVoList = response.data
        })
    },

    selecting(index, depcode) {
    ...
    schedule(depcode) {
        ...
        loginApi.getUserInfo().then(response => {
            ...
            window.location.href = '/user'  //返回index.vue
            return
            }
        })

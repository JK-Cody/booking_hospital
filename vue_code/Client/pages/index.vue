...
import dictApi from '@/api/hospitalDictionary'
...
export default {
  ...
  asyncData({ params, error }) {
    //调用
    return hospApi.getHospitalList(1,10,null)
      .then(response => {
        return {
          list: response.data.content,     //获取内容
          pages: response.data.totalPages  //获取总页数
        }
      })
  },
  data() {
  ...
  methods:{
    ...
    init() {
      //查询医院等级列表
      dictApi.getChildDataByDictCode('Hostype')
        .then(response => {
          //hostypeList清空
          this.hostypeList = []
          //向hostypeList命名键值对
       ...
       //查询默认地区列表
      dictApi.getChildDataByDictCode('Beijing')
        .then(response => {
          this.districtList = []
          //向hostypeList命名键值对
     ...
     //查询医院列表
    getHospitalList() {
      hospApi.getHospitalList(this.page,this.limit,this.searchObj)
        .then(response => {
    ...
    handleSelect(item) {
      window.location.href = '/hospital/' + item.hoscode  //动态路由
    },

    //点击某个医院名称，跳转到详情页面中
    show(hoscode) {
      window.location.href = '/hospital/' + hoscode      //动态路由，以参数名定义跳转页名
    }
  }
}

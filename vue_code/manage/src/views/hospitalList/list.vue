...
<el-table v-loading="listLoading" :data="list"
    ...
    <el-table-column label="医院操作" width="230" align="center">
    <template slot-scope="scope">
        <router-link :to="'/hospitalList/showHospital/'+scope.row.id">
         <el-button type="primary" size="mini">查看</el-button>
        </router-link>

         <router-link :to="'/hospitalList/schedule/'+scope.row.hoscode">
...
export default {
    ...
    methods: {
        ...
        fetchData(page = 1) {
            ...
            // 异步获取远程数据（ajax）
            this.page = page
            hospitalApi.hospitalList(this.page, this.limit, this.searchObj).then(
                response => {
                    //获取java分页方法的变量
         ...
         // 显示登记列表数据
        ChildData() {
           hospitalApi.findChildDataByDictCode('Province')
           .then(response =>{
        ...
        selectProvince() {
            ...
            hospitalApi.findChildData(this.searchObj.provinceCode)
              .then(response => {
         ...
         selectCity() {
            ...
            this.districtList = []
            this.searchObj.districtCode = null
            hospitalApi.findChildData(this.searchObj.cityCode)
              .then(response => {
            this.cityList = response.data.data
        })
        }

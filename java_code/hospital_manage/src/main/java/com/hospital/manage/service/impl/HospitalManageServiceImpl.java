...
import com.hospital.manage.util.BeanUtils;
...
public class HospitalManageServiceImpl implements HospitalManageService {
    ...
    /**
     * 获取院管理的页面参数，然后保存，service_hospital模块会同时响应请求/api/hosp/saveHospital,使用saveHospital方法处理
     * @param data
     * @return
     */
    @Override
    public boolean saveHospital(String data) {
        ...
        //对请求的数据进行加密
        paramMap.put("sign", DigestUtils.md5Hex(this.getSignKey()));
//         paramMap.put("sign", HttpRequestHelper.getSign(paramMap, this.getSignKey()));

        //发起请求，得到service_hospital模块响应
        ...
        if(null != respone && 200 == respone.getIntValue("code")) {
            ...
            throw new manageException(respone.getString("message"), 201);
        }
    }
...
//        paramMap.put("sign", HttpRequestHelper.getSign(paramMap, this.getSignKey()));
    ...
    public boolean saveSchedule(String data) {
        ...
        for(int i=0, len=jsonArray.size(); i<len; i++) {
            ...
            Long id = jsonObject.getLong("scheduleId");
            ...
            paramMap.put("scheduleId",schedule.getId());
            paramMap.put("timestamp", HttpRequestHelper.getTimestamp());
            paramMap.put("sign", DigestUtils.md5Hex(this.getSignKey()));
//            paramMap.put("sign",HttpRequestHelper.getSign(paramMap, this.getSignKey()));

    ...
    <div class="app-container">
        ...
        stripe
            ...
            <el-table-column label="状态" prop="param.statusString">   
                <!-- 状态的颜色改变 -->
                <template slot-scope="scope">                       
                   <p v-if="scope.row.status===0" style="color: red">锁定</p>
     ...
     <el-table-column label="操作" width="200" align="center">
        ...
        <el-button v-if="scope.row.status == 1" type="primary" size="mini" @click="lockUser(scope.row.id, 0)">锁定</el-button>
        <el-button v-if="scope.row.status == 0" type="danger" size="mini" @click="lockUser(scope.row.id, 1)">取消锁定</el-button>
        </template>
    </el-table-column>
    ...
    lockUser(id, status) {
        ...
        this.$confirm('确定该操作吗?', '提示', {
            ...
            type: 'warning'
                ...
                // 点击确定，远程调用ajax
                return clientApi.lockUser(id, status)
            }).then((response) => {

    ...
    <div class="header-container">
        ...
        <!-- logo -->
            <div class="left-wrapper v-link selected">
                <img style="width: 60px" width="50" height="50" src="~assets/images/logo.png">
                <span class="v-link myheaderClickable" @click="goToIndex">预约挂号统一平台</span>
            </div>
       ...
       <div class="container">
        ...
        <div class="operate-view" v-if="dialogAtrr.showLoginType === 'phone'">
          <div class="wrapper" style="width: 100%">
            ...
            <div class="bottom">
              <div class="wechat-wrapper" @click="wechatLogin()"><span
        ...
        <!-- 微信登录 #start -->
        <div class="operate-view"  v-if="dialogAtrr.showLoginType === 'wechat'" >
...
export default {
  ...
  document.getElementById("loginDialog").click();
   ...
   //初始化微信js
    ...
    script.src = 'https://res.wx.qq.com/connect/zh_CN/htmledition/js/wxLogin.js'  //微信平台专有js
    document.body.appendChild(script)
    // 已使用手机登录后，微信登录回调处理openid
    let self = this;
  ...
  methods: {
    ...
    login() {
      ...
      if (this.userInfo.code.length != 6) {
        this.$message.error('验证码格式不正确')   //限制验证码格式
        return;
      }
      this.dialogAtrr.loginBtn = '正在提交...'
      loginApi.login(this.userInfo).then(response => {
    ...
    // 获取验证码
    getCodeFun() {
      if (!(/^1[34578]\d{9}$/.test(this.userInfo.phone))) {   //限制手机号格式
    ...
    wechatLogin() {
      this.dialogAtrr.showLoginType = 'wechat'
      //微信登录
      wechatApi.getLoginParam().then(response => {
      var obj = new WxLogin({  //创建微信登录信息对象
        self_redirect:true,

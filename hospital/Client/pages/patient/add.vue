...
import dictionarApi from '@/api/hospitalDictionary'
...
export default {
    data() {
        return {
            ...
            props: {
                ...
                const { level } = node
                //异步获取省市区
                dictionarApi.getChildData(level ? node.value : '86').then(response => {
    ...
    methods: {
        ...
        fetchDataById(id) {
            patientApi.getPatient(id).then(response => {
        ...
        getDict() {
            dictionarApi.getChildDataByDictCode("CertificatesType").then(response => {
        ...
        saveData() {
            ...
            this.submitBnt = '正在提交...'
                patientApi.savePatient(this.patient).then(response => {
        ...
        updateData() {
            ...
            this.submitBnt = '正在提交...'
            patientApi.updatePatientById(this.patient).then(response => {
                this.$message.success("提交成功")
                window.location.href = '/patient'
            }).catch(e => {
                this.submitBnt = '保存'

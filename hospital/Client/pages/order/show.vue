      ...
      <div class="order-detail">
        ...
        <div class="bottom-wrapper mt60" v-if="orderInfo.orderStatus == 0 || orderInfo.orderStatus == 1">
          ...
          <div class="button-wrapper ml20" v-if="orderInfo.orderStatus == 0">
            <div class="v-button" @click="orderPaymentByWechat()">支付</div>
          </div>
        </div>
      </div>
    </div>
        ...
        <div class="operate-view" style="height: 350px;">
          <div class="wrapper wechat">
            ...
            <div>
              ...
              <!-- 生成支付二维码 -->
               <qriously :value="payObj.codeUrl" :size="200"/>   
...
import bookingOrderApi from '@/api/bookingOrder'
...
export default {
  data() {
    return {
      ...
      dialogPayVisible: false,  //弹窗的显示与否
      payObj: {},
      timer: null  // 定时器名称
    }
  },
  ...
  methods: {
    init() {
      bookingOrderApi.getOrder(this.orderId).then(response => {
        console.log(response.data);
        this.orderInfo = response.data
      })
    },
  //订单支付
  orderPaymentByWechat() {
    this.dialogPayVisible = true  //当弹窗为true时
      wechatApi.createWechatPayment(this.orderId).then(response => {
        this.payObj = response.data
        if(this.payObj.codeUrl == '') {
          this.dialogPayVisible = false
          this.$message.error("支付错误")
        } else {
          this.timer = setInterval(() => {   //调用计时器，每3秒作支付状态查询（持续）
            this.getWechatPaymentStatus(this.orderId)
          }, 3000);  
        }
    })
  },
  ...
  getWechatPaymentStatus(orderId) {
      wechatApi.getWechatPaymentStatus(orderId).then(response => {
        if (response.message == '支付中') {
          return;
        }
        clearInterval(this.timer);  //停止调用计时器
  ...
  this.$confirm('确定取消预约吗?', '提示', {
    ...
    return bookingOrderApi.cancelOrder(this.orderId)
  }).then((response) => {
